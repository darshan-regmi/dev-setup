...
show_summary() {
  ...
  echo -e "  ${CYAN}cleanup${RESET}      Clear Xcode junk + npm cache (Gemini)"
  echo -e "  ${CYAN}l-gpu${RESET}        Monitor GPU/RAM pressure (Gemini)"
  echo -e "  ${CYAN}memcheck${RESET}     Swap usage + memory pressure (Claude)"
  echo -e "  ${CYAN}stop-mongo${RESET}   Quick pkill MongoDB (Gemini)"
  echo -e "  ${CYAN}start-mongo${RESET}  Config-based MongoDB start (Gemini)"
  echo -e "  ${CYAN}mongoup${RESET}      Path-based MongoDB start (Claude)"
